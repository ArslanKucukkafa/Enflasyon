...
from productCartModel import productCart
...
async def signup(request: RequestSchema, db: Session = Depends(get_db)):
    try:
        ...
        print(_user.username,_user.password,_user.email)
...
@router.post("api/breakingToken/")
async def breaking(request:RequestSchema):
     return JWTBearer.jwtBreak(request.parameter.data["token"])

async def breakingRec(request:RequestSchema):
     return JWTBearer.jwtBreak(request.parameter.data["token"])

@router.get("/api/users/",dependencies=[Depends(JWTBearer())])
async def retrieve_all(db: Session = Depends(get_db)):
    print(" User infor ",Session)
